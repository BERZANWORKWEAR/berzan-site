    ...
    drawer.innerHTML = `
      ...
      <div class="cart-summary">
        ...
        <div class="cart-actions">
          <button class="btn primary" id="cartRequest" type="button">Talep gönder</button>
        </div>
      </div>
    `;
...
// Request / Quote modal (sends email via backend if available)
let requestModal;
function ensureRequestModal(){
  if(requestModal) return;
  requestModal = document.createElement('div');
  requestModal.className = 'reqmodal';
  requestModal.innerHTML = `
    <div class="reqmodal-backdrop" id="reqBackdrop" aria-hidden="true"></div>
    <div class="reqmodal-panel" role="dialog" aria-modal="true" aria-labelledby="reqTitle" aria-hidden="true">
      <div class="reqmodal-head">
        <div>
          <div class="reqmodal-eyebrow">Talep özeti</div>
          <div class="reqmodal-title" id="reqTitle">Sipariş talebi</div>
        </div>
        <button class="icon-btn" id="reqClose" type="button" aria-label="Kapat">✕</button>
      </div>

      <div class="reqmodal-form">
        <div class="reqgrid">
          <label class="field">
            <span>Ad Soyad</span>
            <input id="reqName" type="text" autocomplete="name" placeholder="Ad Soyad" />
          </label>
          <label class="field">
            <span>Telefon</span>
            <input id="reqPhone" type="tel" autocomplete="tel" placeholder="05xx xxx xx xx" />
          </label>
          <label class="field span2">
            <span>E-posta (opsiyonel)</span>
            <input id="reqEmail" type="email" autocomplete="email" placeholder="mail@ornek.com" />
          </label>
        </div>

        <div class="reqbox">
          <div class="reqbox-title">İşlem</div>
          <pre class="reqbox-body" id="reqText"></pre>
        </div>

        <div class="reqmodal-actions">
          <button class="btn" id="reqCopy" type="button">Kopyala</button>
          <button class="btn" id="reqDownload" type="button">PDF indir</button>
          <button class="btn primary" id="reqSend" type="button">Mail gönder</button>
        </div>
        <div class="reqhint" id="reqHint"></div>
      </div>
    </div>
  `;
  document.body.appendChild(requestModal);

  const bd = requestModal.querySelector('#reqBackdrop');
  const cl = requestModal.querySelector('#reqClose');
  bd.addEventListener('click', closeRequestModal);
  cl.addEventListener('click', closeRequestModal);

  requestModal.querySelector('#reqCopy').addEventListener('click', async () => {
    const t = requestModal.querySelector('#reqText').textContent || '';
    try{
      await navigator.clipboard.writeText(t);
      setReqHint('Kopyalandı.');
    }catch(e){
      setReqHint('Kopyalanamadı. Metni seçip kopyalayabilirsin.');
    }
  });

  requestModal.querySelector('#reqDownload').addEventListener('click', () => {
    downloadRequestPdf();
  });

  requestModal.querySelector('#reqSend').addEventListener('click', async () => {
    await sendRequestEmail();
  });
}

function setReqHint(msg){
  const el = requestModal?.querySelector('#reqHint');
  if(!el) return;
  el.textContent = msg || '';
}
...
function buildRequestText(){
  ...
  const lines = cart.map(i => {
    ...
    const fmt = (n) => (typeof formatMoney === 'function') ? formatMoney(n) : `₺ ${Math.round(n)}`;
    return `${adet} × ${kod} & ${ad} & ${fmt(birim)} = ${fmt(toplam)}`;
  });
  return lines.join('\n');
}
...
function openRequestModal(){
  ensureRequestModal();
  const panel = requestModal.querySelector('.reqmodal-panel');
  const backdrop = requestModal.querySelector('.reqmodal-backdrop');
  requestModal.querySelector('#reqText').textContent = buildRequestText();
  setReqHint('');
  backdrop.classList.add('open');
...
async function downloadRequestPdf(){
  const payload = {
    to: 'siparis@berzan.com.tr',
    name: requestModal.querySelector('#reqName').value || '',
    phone: requestModal.querySelector('#reqPhone').value || '',
    email: requestModal.querySelector('#reqEmail').value || '',
    items: getCart(),
  ...
  try{
    ...
    downloadBlob('berzan-talep.pdf', blob);
    setReqHint('PDF indirildi.');
  }catch(e){
    // fallback: plain text
    const blob = new Blob([payload.text], {type:'text/plain;charset=utf-8'});
    downloadBlob('berzan-talep.txt', blob);
    setReqHint('Sunucu yoksa: TXT indirildi. (PDF için /api kurulmalı)');
  }
}
...
async function sendRequestEmail(){
  ...
  try{
    ...
    setReqHint('Mail gönderildi ✅');
  }catch(e){
    setReqHint('Mail gönderilemedi (backend lazım). PDF indirip mail ile ekleyebilirsin.');
  }
}
  ...
  const reqBtn = document.getElementById('cartRequest');
  reqBtn?.addEventListener('click', (e)=>{ e.preventDefault(); openRequestModal(); });
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", ensureCartUI, { once: true });
} else {
  ensureCartUI();
}
...
function initShopPage(){
  ...
  function updateChips(){
    ...
    if (sectorParam) {
      ...
      if (sectorChip) sectorChip.textContent = `Sektör: ${BERZAN_SECTOR_MAP[sectorParam] || sectorParam}`;
    } else {
    ...
    if (activeSeason) {
      ...
      if (seasonChip) seasonChip.textContent = `Sezon: ${seasonLabel()}`;
    } else {
    ...
    if (query) {
      ...
      if (queryChip) queryChip.textContent = `Arama: ${query}`;
    } else {
  ...
  function applyAll(){
    ...
    if (rightNote) rightNote.textContent = parts.length ? parts.join(" • ") : "";
...
function ensureCookieConsent(){
  try{
    const KEY = 'berzan_cookie_consent_v1';
    ...
    bar.innerHTML = `
      <div class="consent-inner">
        <div class="consent-text">
          Bu sitede hizmeti geliştirmek için çerezler kullanıyoruz. Detaylar: <a href="./kvkk.html">KVKK</a> ve <a href="./cerez-politikasi.html">Çerez Politikası</a>.
        </div>
        <div class="consent-actions">
          <button class="btn" type="button" data-consent-reject>Reddet</button>
          <button class="btn primary" type="button" data-consent-accept>Kabul et</button>
        </div>
      </div>
    `;
    ...
    const accept = bar.querySelector('[data-consent-accept]');
    const reject = bar.querySelector('[data-consent-reject]');
